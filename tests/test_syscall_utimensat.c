...
int main() {
    ...
    // Set up mock current task so vmm validation won't crash
    memset(&current_task_mock, 0, sizeof(task_t));
    current_task_mock.id = 1;

    // Call directly instead of through dispatch table
    int64_t ret = sys_utimensat(-1, "/test", ts, 0);

    if (ret == 0) {
        printf("PASSED: sys_utimensat returned success\n");
    } else {
        printf("FAILED: sys_utimensat returned %ld\n", ret);
        return 1;
    }
...
int eteros_strcmp(const char* s1, const char* s2) {
    while (*s1 && (*s1 == *s2)) {
        s1++;
        s2++;
    }
