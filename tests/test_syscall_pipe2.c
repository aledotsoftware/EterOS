...
int vmm_verify_user_access(const void* addr, size_t size, int write) {
    if ((uintptr_t)addr >= 0xFFFF000000000000) return 0;
    return 1;
}
...
int main() {
    printf("Running test_syscall_pipe2...\n");

    // Setup task
    memset(&current_task_mock, 0, sizeof(task_t));
    current_task_mock.id = 1;

    // We don't have pipe creation mock fully hooked up, but we can call sys_pipe2 and check if it handles bad pointers
    int64_t pipe_res = sys_pipe2((int*)0xFFFFFFFFFFFFFFFF, 0);

    if (pipe_res == -EFAULT) {
        printf("PASSED: sys_pipe2 rejected bad pointer\n");
    } else {
        printf("FAILED: sys_pipe2 accepted bad pointer returning %ld\n", pipe_res);
        return 1;
    }
