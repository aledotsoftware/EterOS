...
int mock_readdir(fs_node_t *node, uint32_t index, struct dirent *entry) {
    (void)node;
    if (index == 0) {
        strlcpy(entry->name, ".", 127);
        entry->inode = 100;
        return 0; // Success
    } else if (index == 1) {
        strlcpy(entry->name, "file.txt", 127);
        entry->inode = 101;
        return 0; // Success
    }
...
int readdir_fs(fs_node_t *node, uint32_t index, struct dirent *entry) {
    ...
    if (node->readdir != NULL) {
        return node->readdir(node, index, entry);
    }
    return -1;
}
...
fs_node_t* finddir_fs(fs_node_t* node, char* name) {
    ...
    if (strcmp(name, ".") == 0) ret->flags = FS_DIRECTORY;
    else if (strcmp(name, "file.txt") == 0) ret->flags = FS_FILE;
    return ret;
}
...
int main() {
    printf("Running test_syscall_getdents64...\n");
    ...
    current_task_mock.fd_table[3].node = node;
    current_task_mock.fd_table[3].offset = 0;

    // Test sys_getdents64
    char buf[1024];
    memset(buf, 0, sizeof(buf));

    int64_t res = sys_getdents64(3, (struct linux_dirent64*)buf, sizeof(buf));

    if (res > 0) {
        printf("PASSED: sys_getdents64 returned %ld bytes\n", res);
    } else {
        printf("FAILED: sys_getdents64 returned %ld\n", res);
        return 1;
    }
