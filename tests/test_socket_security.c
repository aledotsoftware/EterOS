...
int sys_lwip_accept(int fd, struct sockaddr* addr, socklen_t* addrlen) { return -14; /* EFAULT */ }
int sys_lwip_listen(int fd, int backlog) { return -1; }
ssize_t sys_lwip_sendto(int fd, const void* buf, size_t len, int flags, const struct sockaddr* dest_addr, socklen_t addrlen) { return -14; /* EFAULT */ }
ssize_t sys_lwip_recvfrom(int fd, void* buf, size_t len, int flags, struct sockaddr* src_addr, socklen_t* addrlen) { return -14; /* EFAULT */ }
...
int main() {
    ...
    if (ret != -EFAULT) {
        printf("FAILED: sys_accept did not return -EFAULT for bad addr (ret=%ld)\n", ret);
        return 1;
    }
