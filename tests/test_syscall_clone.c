...
int main() {
    ...
    int64_t clone_res = sys_clone(0, 0, 0, 0, 0, &regs);

    // It should call task_fork returning 0
    if (clone_res == 0) {
        printf("PASSED: sys_clone called task_fork successfully\n");
    } else {
        printf("FAILED: sys_clone returned %ld\n", clone_res);
        return 1;
    }
