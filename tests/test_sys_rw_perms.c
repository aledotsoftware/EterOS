...
int sys_lwip_connect(int fd, const struct sockaddr* addr, socklen_t addrlen) { return -1; }
int sys_lwip_bind(int fd, const struct sockaddr* addr, socklen_t addrlen) { return -1; }
...
ssize_t sys_lwip_sendto(int fd, const void* buf, size_t len, int flags, const struct sockaddr* dest_addr, socklen_t addrlen) { return -1; }
ssize_t sys_lwip_recvfrom(int fd, void* buf, size_t len, int flags, struct sockaddr* src_addr, socklen_t* addrlen) { return -1; }
...
int sys_lwip_getsockname(int fd, struct sockaddr* addr, socklen_t* addrlen) { return -1; }
int sys_lwip_getpeername(int fd, struct sockaddr* addr, socklen_t* addrlen) { return -1; }
...
int main() {
    printf("Running test_sys_rw_perms...\n");
    ...
    current_task_mock.fd_table[0].offset = 0;

    current_task_mock.fd_table[1].node = &dummy_node;
    current_task_mock.fd_table[1].flags = O_WRONLY;
    current_task_mock.fd_table[1].offset = 0;

    current_task_mock.fd_table[2].node = &dummy_node;
    current_task_mock.fd_table[2].flags = O_RDWR;
    current_task_mock.fd_table[2].offset = 0;

    char buf[10];

    // O_RDONLY tests
    if (sys_read(0, buf, 10) < 0) {
    ...
    if (sys_write(1, buf, 10) < 0) {
        printf("FAILED: sys_write on O_WRONLY failed\n");
        return 1;
    }

    // O_RDWR tests
    if (sys_read(2, buf, 10) < 0) {
        printf("FAILED: sys_read on O_RDWR failed\n");
        return 1;
    }
    if (sys_write(2, buf, 10) < 0) {
        printf("FAILED: sys_write on O_RDWR failed\n");
        return 1;
    }
