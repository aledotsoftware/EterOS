...
/* Mock vfs_lookup to simulate file and directory */
...
int main() {
    ...
    /* fd_table initialized to NULLs */

    /* Test 1: Open file with O_RDONLY (Should succeed) */
    printf("Test 1: Open /file O_RDONLY... ");
    int64_t fd = sys_open("/file", O_RDONLY, 0);
    if (fd >= 0) {
        printf("PASSED (fd=%ld)\n", fd);
        /* Cleanup fd */
        if (current_task_mock.fd_table[fd].node) {
            free(current_task_mock.fd_table[fd].node);
            current_task_mock.fd_table[fd].node = NULL;
        }
    } else {
    ...
    /* Test 2: Open directory with O_RDONLY (Should succeed) */
    printf("Test 2: Open /dir O_RDONLY... ");
    fd = sys_open("/dir", O_RDONLY, 0);
    if (fd >= 0) {
        printf("PASSED (fd=%ld)\n", fd);
        /* Cleanup fd */
        if (current_task_mock.fd_table[fd].node) {
    ...
    /* Test 3: Open directory with O_WRONLY (Should fail with -EISDIR) */
    printf("Test 3: Open /dir O_WRONLY... ");
    fd = sys_open("/dir", O_WRONLY, 0);
    if (fd == -EISDIR) {
        printf("PASSED (-EISDIR)\n");
    } else {
        printf("FAILED (ret=%ld, expected -21/EISDIR)\n", fd);
        if (fd >= 0) {
             /* Cleanup leaked fd if test fails */
             if (current_task_mock.fd_table[fd].node) {
                free(current_task_mock.fd_table[fd].node);
                current_task_mock.fd_table[fd].node = NULL;
            }
    ...
    /* Test 4: Open directory with O_RDWR (Should fail with -EISDIR) */
    printf("Test 4: Open /dir O_RDWR... ");
    fd = sys_open("/dir", O_RDWR, 0);
    if (fd == -EISDIR) {
        printf("PASSED (-EISDIR)\n");
    } else {
        printf("FAILED (ret=%ld, expected -21/EISDIR)\n", fd);
