...
int main() {
    printf("Running test_syscall_rt_sigaction...\n");

    // Setup task
    memset(&current_task_mock, 0, sizeof(task_t));
    current_task_mock.id = 1;

    // Call sys_rt_sigaction with invalid sig
    int64_t sig_res = sys_rt_sigaction(65, NULL, NULL, 8);

    if (sig_res == -EINVAL) {
        printf("PASSED: sys_rt_sigaction rejected invalid signal\n");
    } else {
        printf("FAILED: sys_rt_sigaction accepted invalid signal returning %ld\n", sig_res);
        return 1;
    }
