...
static void cmd_net(const char* args) {
    ...
    terminal_write_string("\n");
    terminal_write_colored("  Interfaz de Red (eth0):\n", VGA_COLOR_LIGHT_CYAN, VGA_COLOR_BLACK);
    
    terminal_write_string("    MAC:      ");
    for(int i=0; i<6; i++) {
        utoa_hex_s(mac[i], buf, sizeof(buf));
        terminal_write_string(buf);
    ...
    if (my_ip == 0) {
        terminal_write_colored("0.0.0.0 (Sin Asignar)", VGA_COLOR_DARK_GREY, VGA_COLOR_BLACK);
    } else {
        itoa_s(ip[0], buf, sizeof(buf), 10); terminal_write_string(buf); terminal_write_string(".");
        itoa_s(ip[1], buf, sizeof(buf), 10); terminal_write_string(buf); terminal_write_string(".");
        itoa_s(ip[2], buf, sizeof(buf), 10); terminal_write_string(buf); terminal_write_string(".");
        itoa_s(ip[3], buf, sizeof(buf), 10); terminal_write_string(buf);
    }
    terminal_write_string("\n");

    /* Show Gateway */
    uint8_t* gw = (uint8_t*)&gateway_ip;
    terminal_write_string("    Gateway:  ");
    itoa_s(gw[0], buf, sizeof(buf), 10); terminal_write_string(buf); terminal_write_string(".");
    itoa_s(gw[1], buf, sizeof(buf), 10); terminal_write_string(buf); terminal_write_string(".");
    itoa_s(gw[2], buf, sizeof(buf), 10); terminal_write_string(buf); terminal_write_string(".");
    itoa_s(gw[3], buf, sizeof(buf), 10); terminal_write_string(buf);
    terminal_write_string("\n");

    /* Show DNS */
    uint8_t* dns = (uint8_t*)&dns_ip;
    terminal_write_string("    DNS:      ");
    itoa_s(dns[0], buf, sizeof(buf), 10); terminal_write_string(buf); terminal_write_string(".");
    itoa_s(dns[1], buf, sizeof(buf), 10); terminal_write_string(buf); terminal_write_string(".");
    itoa_s(dns[2], buf, sizeof(buf), 10); terminal_write_string(buf); terminal_write_string(".");
    itoa_s(dns[3], buf, sizeof(buf), 10); terminal_write_string(buf);
    terminal_write_string("\n");

    terminal_write_string("    Estado:   ");
    if (network_ready) {
        terminal_write_colored("CONECTADO", VGA_COLOR_GREEN, VGA_COLOR_BLACK);
    } else {
        terminal_write_colored("DESCONECTADO", VGA_COLOR_RED, VGA_COLOR_BLACK);
    }
    terminal_write_string("\n\n");
}
...
static void cmd_dhcp(const char* args) {
    (void)args;
    terminal_write_string("  Solicitando IP via DHCP...\n");
    dhcp_discover();
    /* The loop usually returns after success or timeout */
}

static void cmd_wget(const char* args) {
    if (!args || !*args) {
        terminal_write_string("Uso: wget <IP>\n");
        return;
    }
    /* Skip spaces */
    while (*args == ' ') args++;
    ...
    uint32_t target_ip = ip_aton(args);
    if (target_ip == 0) {
        terminal_write_string("  Resolviendo host '");
        terminal_write_string(args);
        terminal_write_string("'... ");
        
        #include "../include/net/dns.h"
        target_ip = dns_resolve(args);
        
        if (target_ip == 0) {
             terminal_write_colored("FAILED (DNS Error)\n", VGA_COLOR_RED, VGA_COLOR_BLACK);
             return;
        }
        
        char ipbuf[32];
        uint8_t* oct = (uint8_t*)&target_ip;
        itoa_s(oct[0], ipbuf, sizeof(ipbuf), 10); terminal_write_string(ipbuf); terminal_write_string(".");
        itoa_s(oct[1], ipbuf, sizeof(ipbuf), 10); terminal_write_string(ipbuf); terminal_write_string(".");
        itoa_s(oct[2], ipbuf, sizeof(ipbuf), 10); terminal_write_string(ipbuf); terminal_write_string(".");
        itoa_s(oct[3], ipbuf, sizeof(ipbuf), 10); terminal_write_string(ipbuf);
        terminal_write_string("\n");
    }

    if (!network_ready || my_ip == 0) {
        terminal_write_colored("Error: Red no configurada. Ejecuta 'dhcp' primero.\n", VGA_COLOR_RED, VGA_COLOR_BLACK);
        return;
    }

    terminal_write_string("  Conectando a ");
    terminal_write_string(args);
    terminal_write_string("... ");

    int fd = net_socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
    if (fd < 0) {
        terminal_write_string("Error: Socket failed.\n");
        return;
    }
    ...
    if (net_connect(fd, &addr, sizeof(addr)) != 0) {
        terminal_write_colored("FAILED\n", VGA_COLOR_RED, VGA_COLOR_BLACK);
        net_close(fd);
        return;
    }
    terminal_write_colored("OK\n", VGA_COLOR_GREEN, VGA_COLOR_BLACK);

    terminal_write_string("  Enviando Request... ");
    char req[] = "GET / HTTP/1.0\r\nUser-Agent: eterOS\r\n\r\n";
    if (net_send(fd, req, sizeof(req)-1, 0) < 0) {
        terminal_write_string("Error send.\n");
        net_close(fd);
        return;
    }
    terminal_write_string("Enviado.\n");

    terminal_write_string("  Recibiendo datos...\n");
    terminal_write_string("  ------------------------------------\n");
    
    char buf[512];
    int len;
    while ((len = net_recv(fd, buf, sizeof(buf)-1, 0)) > 0) {
        buf[len] = 0;
        terminal_write_string(buf);
    }
    terminal_write_string("\n  ------------------------------------\n");
    terminal_write_string("  Conexion cerrada.\n");
    net_close(fd);
}
...
static void cmd_usermode(const char* args) {
    const char* filename = "test.elf";
    if (args && *args) {
        while (*args == ' ') args++;
        if (*args) filename = args;
    }

    terminal_write_string("\n  [USER] Loading '");
    terminal_write_string(filename);
    terminal_write_string("' from Initrd...\n");

    /* 1. Try to load ELF */
    /* Initrd is mounted at / */
    uint64_t entry_point = elf_load_file(filename, 0x200000000); // 8GB mark (User Space)

    if (entry_point == 0 && args && *args) {
        /* If user provided name failed, try adding / */
        char path[64];
        if (filename[0] != '/') {
            strlcpy(path, "/", sizeof(path));
            strlcat(path, filename, sizeof(path));
            entry_point = elf_load_file(path, 0x200000000);
        }
    }

    if (entry_point == 0) {
        terminal_write_colored("  Error: Could not load ELF. File not found or invalid.\n", VGA_COLOR_RED, VGA_COLOR_BLACK);
        /* Fallback: try /initrd/test.elf just in case */
        if (strcmp(filename, "test.elf") == 0) {
             entry_point = elf_load_file("/initrd/test.elf", 0x200000000);
    ...
    terminal_write_string("  [USER] ELF Loaded. Entry point: 0x");
    char buf[32];
    ...
    for (int i = 0; i < 2; i++) {
        ...
        hal_mem_map((uint64_t)stack_phys, user_stack_virt - (i * PAGE_SIZE), HAL_MEM_READ | HAL_MEM_WRITE | HAL_MEM_USER);
    }

    /* Stack Top (grows down from 0x300001000) */
    // Wait, if I map at 0x300000000 and 0x2FFFFF000 (stack grows down)
    // The top valid address is 0x300000FFF. 
    // SP should be 0x300001000? 
    // Let's stick to what user_loader did: user_stack_virt + PAGE_SIZE
    
    uint64_t stack_top = user_stack_virt + PAGE_SIZE;

    terminal_write_string("  [USER] Jump to Ring 3...\n");

    /* 3. Enter User Mode */
    enter_user_mode((void*)entry_point, (void*)stack_top);

    /* Should never return */
    terminal_write_string("  [USER] Error: Returned from Ring 3.\n");
}
