...
int sys_connect(int fd, const struct sockaddr_old* addr, int addrlen) {
...
int sys_bind(int fd, const struct sockaddr_old* addr, int addrlen) {
    if (!vmm_verify_user_access(addr, addrlen, 0)) return -EFAULT;
    if (addrlen < (int)sizeof(struct sockaddr_in_old)) return -EINVAL;
    task_t* current = task_get_current();
    if (fd < 0 || fd >= MAX_FD) return -EBADF;
...
int sys_accept4(int fd, struct sockaddr_old* addr, int* addrlen, int flags) {
    (void)flags;
    return sys_accept(fd, addr, addrlen);
}

int sys_sendto(int fd, const void* buf, size_t len, int flags, const struct sockaddr_old* dest_addr, int addrlen) {
...
int sys_recvfrom(int fd, void* buf, size_t len, int flags, struct sockaddr_old* src_addr, int* addrlen) {
...
int sys_getpeername(int fd, struct sockaddr_old* addr, int* addrlen) {
    (void)fd; (void)addr; (void)addrlen;
    return -EOPNOTSUPP;
}

int sys_getsockname(int fd, struct sockaddr_old* addr, int* addrlen) {
    (void)fd; (void)addr; (void)addrlen;
    return -EOPNOTSUPP;
}
