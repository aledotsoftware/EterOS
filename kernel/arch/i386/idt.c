...
/* ========================================================================= */
/* Tabla IDT (256 entradas × 8 bytes = 2 KB)                                */
/* ========================================================================= */
...
/**
 * Configura una entrada de la IDT con el handler dado (32-bit).
 */
...
#define EXCEPTION_HANDLER_ERR(num) \
    __attribute__((interrupt)) \
    static void isr_##num(struct interrupt_frame *frame, uint32_t error_code) { \
        (void)frame; \
        (void)error_code; \
        handle_exception(num); \
    }
...
void idt_init(void) {
    ...
    serial_write_string("[eterOS/i386] IDT cargada (256 vectores).\n");
}
