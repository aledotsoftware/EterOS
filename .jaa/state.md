...
- **vfs-posix-filesystem-bot**: Enforced strict POSIX semantics across the Virtual File System stack.
  - Replaced generic negative error codes (`-1`, `-2`, `-3`) with standardized POSIX constants (e.g., `-EINVAL`, `-ENOENT`, `-ENOMEM`, `-EEXIST`, `-ENOSPC`, `-ENOTDIR`) in VFS wrappers (`vfs.c`) and backend implementations (`jfs.c`, `fat32.c`, `shmfs.c`, `devfs.c`, `bcache.c`, `procfs.c`).
  - Fixed internal backend functions (e.g. `fat32_alloc_cluster`, `jfs_ensure_dir_block`) to correctly bubble up specific POSIX errors to the caller rather than squashing them, resolving unexpected system behavior on boundary failures.
