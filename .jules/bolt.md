...
## 2026-12-06 - [Fixed-Size Hex Formatting]
**Learning:** Formatting functions with fixed maximum output lengths (like `utoa_hex_s` for 64-bit integers) incur significant overhead from branching, loop control, and break conditions when using standard iteration. Since a 64-bit hex string is exactly 16 characters plus "0x" prefix, loop logic can be entirely eliminated.
**Action:** When a loop has a small, statically known maximum number of iterations (e.g., 16 digits), unroll it completely to eliminate branch instructions and loop maintenance overhead. This specific optimization yielded a ~45x speedup for hex formatting.
